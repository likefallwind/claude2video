...
Optionally, decide whether per-section illustrations should be generated by the AI image tool (`gen_images.py --section-illustrations`). Set this flag when:
- The topic involves real-world phenomena that benefit from visual context (physics experiments, biology structures, chemistry reactions)
- The storyboard has sections that would benefit from a themed background illustration

Output `section_illustrations: true` or `section_illustrations: false` at the end of the assets output.

### Output Format
- Output asset keywords in the format `keyword: visual description` (one per line).
  - The description helps `gen_images.py` generate better images.
  - Example: `basketball: orange basketball in mid-air, sports equipment`
  - Example: `microscope: laboratory microscope, silver and black, scientific instrument`
- If no description is available, plain keywords are also accepted.
- At the end, add a line: `section_illustrations: true` or `section_illustrations: false`.
